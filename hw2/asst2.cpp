...
static Matrix4 g_objectRbt[2] = {Matrix4::makeTranslation(Cvec3(-0.8,0,0)), Matrix4::makeTranslation(Cvec3(0.8,0,0))};  // currently only 1 obj is defined
...
static Matrix4 g_currentRbt = g_skyRbt;
static Matrix4 g_worldRbt = Matrix4::makeTranslation(Cvec3(0.0, 0.0, 0.0)); // temp
...
static void drawStuff() {
  ...
  const Matrix4 eyeRbt = g_currentRbt;//g_skyRbt;
      ...
      if (g_currentObjCode == 0)
      {
          if (g_currentEyeCode == 0)
          {
              ...
              else if (g_skymode == 1) // sky-sky frame
              {
                  ...
                  Matrix4 m1 = transFact(m);
                  Matrix4 m2 = inv(linFact(m));
