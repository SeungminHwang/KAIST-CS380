...
bool interpolateAndDisplay(float t) {
    ...
    list<vector<RigTForm> >::iterator it1, it2, temp_begin, temp_end;

    temp_begin = g_keyframes.begin();
    temp_end = g_keyframes.end();

    it1 = it2 = ++temp_begin;// g_keyframes.begin();
    advance(it1, floor_t);
    advance(it2, floor_t + 1);
    if (it2 == --temp_end)
    {
        //cout << "here!" << endl;
        return true;
    ...
    vector<RigTForm> F0 = *it1;
    vector<RigTForm> F1 = *it2;

    vector<RigTForm>::iterator it_a0 = F0.begin();
    vector<RigTForm>::iterator it_a1 = F1.begin();

    // do the interpolation task for all Rbts
    for (vector<shared_ptr<SgRbtNode>>::iterator it = g_allRbts.begin(); it != g_allRbts.end(); ++it, ++it_a0, ++it_a1)
    {
        ...
        double temp_cos = temp[0];
        //cout << "temp_cos: " << temp_cos << endl;
        double temp_sin = sqrt(1 - temp_cos * temp_cos);
        ...
        {//cout << "theta: " << theta << endl;
            powered = Quat(cos(theta * alpha), 0, 0, 0);
        }
        else
        {
            powered = Quat(cos(theta * alpha), sin(theta * alpha) * (temp[1] / temp_sin),
                sin(theta * alpha) * (temp[2] / temp_sin), sin(theta * alpha) * (temp[3] / temp_sin));
        }
       
        Quat R = powered*R0;

        //float phi = atan2(sqrt(temp[1]* temp[1] + temp[2] * temp[2] + temp[3] * temp[3]), temp[0]);
        //cout << "quat" << temp[0] << " " << temp[1] << " " << temp[2] << " " << temp[3] << endl;
        //Quat R = Quat(cos(phi*alpha),sin(phi*alpha)*temp[1], sin(phi * alpha)*temp[2], sin(phi * alpha)*temp[3])*R0;
        //Quat R = Quat(1, 0, 0, 0);
