...
// dynamically changing mesh
Mesh g_mesh;
static int g_division_count = 0;
static int g_shading_mode = 1;
static float g_deforming_speed = 1;
...
static vector<VertexPN> simpleMeshShading(Mesh mesh) {
    vector<VertexPN> vertices;
    for (int i = 0; i < mesh.getNumFaces(); i++)
    {
        Mesh::Face face = mesh.getFace(i);

        Mesh::Vertex mesh_v0 = face.getVertex(0);
        Mesh::Vertex mesh_v1 = face.getVertex(1);
        Mesh::Vertex mesh_v2 = face.getVertex(2);
        Mesh::Vertex mesh_v3 = face.getVertex(3);

        Cvec3 normal = face.getNormal();

        //triangle 1
        vertices.push_back(VertexPN(mesh_v0.getPosition(), normal));
        vertices.push_back(VertexPN(mesh_v1.getPosition(), normal));
        vertices.push_back(VertexPN(mesh_v2.getPosition(), normal));

        //cout << mesh_v0.getIndex() << endl;

        //triangle 2
        vertices.push_back(VertexPN(mesh_v0.getPosition(), normal));
        vertices.push_back(VertexPN(mesh_v2.getPosition(), normal));
        vertices.push_back(VertexPN(mesh_v3.getPosition(), normal));
    }

    return vertices;
}
static vector<VertexPN> smoothShading(Mesh mesh)//(shared_ptr<SimpleGeometryPN> geometry, Mesh& mesh)
{
    vector<int> vertexCount(mesh.getNumVertices()); // initialized with 0

    // initialization (delete previous normals
    ...
    for (int i = 0; i < mesh.getNumFaces(); i++)
    {
        ...
        {
            Mesh::Vertex vj = face.getVertex(j);
            Cvec3 normal = face.getNormal();

            // vertex occurence count ++
            int idx = vj.getIndex();
            vertexCount[idx]++;

            vj.setNormal(vj.getNormal() + normal);
        }
    }
    ...
    for (int i = 0; i < mesh.getNumVertices(); i++)
    {
        Mesh::Vertex vi = mesh.getVertex(i);
        int idx = vi.getIndex();
        vi.setNormal(vi.getNormal() / (float)vertexCount[idx]);
    }

    // mesh to triangle geometry
    vector<VertexPN> vertices;
    for (int i = 0; i < mesh.getNumFaces(); i++)
    {
        Mesh::Face face = mesh.getFace(i);
        ...
        //triangle 1
        vertices.push_back(VertexPN(mesh_v0.getPosition(), mesh_v0.getNormal()));
        vertices.push_back(VertexPN(mesh_v1.getPosition(), mesh_v1.getNormal()));
        vertices.push_back(VertexPN(mesh_v2.getPosition(), mesh_v2.getNormal()));
        ...
        vertices.push_back(VertexPN(mesh_v0.getPosition(), mesh_v0.getNormal()));
        vertices.push_back(VertexPN(mesh_v2.getPosition(), mesh_v2.getNormal()));
        vertices.push_back(VertexPN(mesh_v3.getPosition(), mesh_v3.getNormal()));
    }


    //g_meshObj.reset(new SimpleGeometryPN);
    //g_meshObj->upload(&vertices[0], g_mesh.getNumFaces() * 6);
    return vertices;
}

static void initMeshObj() {

    // read mesh and convert square structure into triangle structure
    g_mesh.load("./cube.mesh");

    vector<VertexPN> vertices = simpleMeshShading(g_mesh);
...
static void subdivision(Mesh &mesh)
{
    ...
    {
        Mesh::Face face = mesh.getFace(i);
        Cvec3 acc = Cvec3(0, 0, 0);
        for (int j = 0; j < face.getNumVertices(); j++)
        {
            acc += face.getVertex(j).getPosition();
        }
        mesh.setNewFaceVertex(face, acc / face.getNumVertices());
    }
    ...
    for (int i = 0; i < mesh.getNumEdges(); i++)
    {
        ...
        acc += edge.getVertex(0).getPosition();
        acc += edge.getVertex(1).getPosition();

        Mesh::Face adjFace1 = edge.getFace(0);
        Mesh::Face adjFace2 = edge.getFace(1);

        acc += mesh.getNewFaceVertex(adjFace1);
        acc += mesh.getNewFaceVertex(adjFace2);

        mesh.setNewEdgeVertex(edge, acc / 4);
    }
    ...
    for (int i = 0; i < mesh.getNumVertices(); i++)
    {
        ...
        // do sum(vj) + sum(v_fj)
        do {
            acc += it.getVertex().getPosition() + mesh.getNewFaceVertex(it.getFace());
            nv++;
        } while (++it != it0);


        acc /= (nv * nv);
        acc += (vertex.getPosition() * (nv - 2) / nv);
        mesh.setNewVertexVertex(vertex, acc);
    }
...
// HW8 - TASK3
static void dynamicVertexCallback(int ms)
{
    float t = (float)ms / (float)g_msBetweenKeyFrames * g_deforming_speed* 0.5;

    Mesh temp = Mesh(g_mesh);

    for (int i = 0; i < temp.getNumVertices(); i++)
    {
        Mesh::Vertex v = temp.getVertex(i);
        v.setPosition(v.getPosition() * (1 + 0.7*sin((i + 1)*t)));
    }

    for (int i = 0; i < g_division_count; i++)
    {
        subdivision(temp);
    }
    ...
    //fix
    //vector<VertexPN> vertices = simpleMeshShading(temp);
    vector<VertexPN> vertices;
    if (g_shading_mode == 1)
    {
        vertices = simpleMeshShading(temp);
    }
    else
    {
        vertices = smoothShading(temp);
    }
    //vector<VertexPN> vertices = smoothShading(temp);


    //g_meshObj.reset(new SimpleGeometryPN);
    g_meshObj->upload(&vertices[0], temp.getNumFaces() * 6);

    //g_meshObj->upload(&vertices[0], g_mesh.getNumFaces() * 6);

    glutPostRedisplay();
    glutTimerFunc(100/g_deforming_speed, dynamicVertexCallback, ms + 100 / g_deforming_speed);
}
...
static void keyboard(const unsigned char key, const int x, const int y) {
    ...
    case '0':
        g_division_count += 1;
        cout << "Subdivision levels = " << g_division_count << endl;
        break;
    case '9':
        g_division_count = max(0, g_division_count - 1);
        cout << "Subdivision levels = " << g_division_count << endl;
        break;
    case 'f':
        g_shading_mode *= -1;
    ...
    case '7':
        g_deforming_speed /= 2;
        cout << "bubbling speed = " << g_deforming_speed << endl;
        break;
    case'8':
        g_deforming_speed *= 2;
        cout << "bubbling speed = " << g_deforming_speed << endl;
...
static void initMaterials() {
    ...
    g_meshObjMat.reset(new Material("./shaders/basic-gl3.vshader", "./shaders/specular-gl3.fshader"));
    g_meshObjMat->getUniforms().put("uColor", Cvec3f(0.8f, 0.8f, 0.2f));
    //temp code
    //g_meshObjMat->getRenderStates().polygonMode(GL_FRONT_AND_BACK, GL_LINE);
};
...
static void initScene() {
    ...
    */
    ...
    //mesh obj
    g_meshObjNode.reset(new SgRbtNode(RigTForm(Cvec3(0, 0.5, 0)) ) );
    g_meshObjNode->addChild(shared_ptr<MyShapeNode>(
        new MyShapeNode(g_meshObj, g_meshObjMat, Cvec3(0, 0, 0))));
    ...
    g_world->addChild(g_meshObjNode);
}
